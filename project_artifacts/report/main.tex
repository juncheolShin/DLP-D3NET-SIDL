...
고정된 최종 SIDL train/validation split에서 256$\times$256 crop으로 continued training한 D3Net은 epoch 73 validation 기준 23.86 dB / SSIM 0.8419를 달성하였다.
...
이후 latest checkpoint에서 256$\times$256 continued training을 수행한 결과 epoch 73에서 23.86 dB / 0.8419의 current best를 얻었다.
...
AirNet과 DiffUIR는 SIDL 논문의 test-set 결과이고, D3Net은 본 프로젝트의 validation-set 결과이므로 완전히 동일한 split의 leaderboard 비교는 아니다.
따라서 이 표는 SOTA 주장보다는 기존 baseline의 성능 범위 안에서 D3Net adaptation 결과가 어떤 difficulty/type에서 강하거나 약한지 해석하기 위한 context로 사용한다.

\input{tables/results_main.tex}

256$\times$256 continued training은 epoch 73에서 23.86 dB / 0.8419로 current best validation score를 보였다.
기존 epoch 34 checkpoint의 23.82 dB / 0.8398과 비교하면 PSNR은 0.04 dB, SSIM은 0.0021 상승하였다.
...
반면 latest checkpoint에서 이어간 256 continued training은 epoch 73에서 PSNR과 SSIM을 동시에 개선했으므로, 현재 보고서의 최종 후보는 epoch 73 checkpoint로 갱신한다.

Epoch 73 checkpoint는 Hard difficulty 복원 성능을 개선하는 대신 Easy difficulty PSNR을 일부 희생하였다.
Easy average는 epoch 34의 27.00 dB / 0.8812에서 epoch 73의 26.66 dB / 0.8810으로 낮아졌지만, Hard average는 20.33 dB / 0.7743에서 20.59 dB / 0.7775로 상승하였다.
이는 all-in-one restoration에서 강한 residual correction이 심한 오염에는 도움이 되지만, 약한 오염 sample에는 over-correction으로 작용할 수 있는 cross-difficulty trade-off를 시사한다.
...
Epoch 73 기준 Easy average는 26.66 dB / 0.8810, Medium average는 25.33 dB / 0.8877, Hard average는 20.59 dB / 0.7775이다.
Type-wise average에서는 clean이 29.87 dB / 0.9125로 가장 높고, mixed가 20.67 dB / 0.8311, finger가 21.97 dB / 0.7897, dust가 21.90 dB / 0.8217로 낮다.
Type-wise 변화에서는 Finger와 Mixed가 각각 +0.17 dB / +0.0039, +0.19 dB / +0.0042로 개선에 기여했고, Clean과 Water의 PSNR은 각각 0.06 dB 감소하였다.
특히 Hard Dust는 17.05 dB / 0.7086, Hard Mixed는 17.98 dB / 0.7900으로 여전히 주요 병목에 해당한다.
...
최종 후보인 epoch 73 continued checkpoint는 256$\times$256 crop setting에서 23.86 dB / SSIM 0.8419를 기록하였다.
이는 기존 epoch 34 checkpoint보다 +0.04 dB PSNR, +0.0021 SSIM 개선된 결과이다.
