...
\author{TODO: 이름 / Deep Learning Programming}
...
중요하게도 본 보고서는 D3Net이 SIDL에서 SOTA를 달성했다고 주장하지 않는다.
대신 D3Net이라는 degradation-adaptive all-in-one backbone을 SIDL에 적용했을 때 어떤 조건에서 안정적으로 학습되는지, 그리고 어떤 조건에서 성능이 무너지는지를 분석하는 adaptation study로 해석한다.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.98\linewidth,height=0.38\textheight,keepaspectratio]{figures/d3net_architecture_paper.pdf}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.78\linewidth,height=0.30\textheight,keepaspectratio]{figures/d3net_ddm_paper.pdf}
...
Validation은 denormalized output과 target을 $[0,1]$ 범위로 clamp한 뒤 PSNR/SSIM을 image-wise로 계산하였다.
...
\input{tables/sidl_airnet_diffuir_baselines.tex}

표 \ref{tab:sidl_airnet_diffuir}는 사용자가 비교 대상으로 지정한 AirNet과 DiffUIR의 SIDL 결과를 정리한 것이다.
두 모델은 SIDL에서 all-in-one 방식으로 학습/평가된 baseline이므로, 본 프로젝트의 D3Net adaptation 결과를 해석하는 데 직접적인 task context를 제공한다.
다만 SIDL 논문의 test setting과 본 프로젝트의 validation setting은 완전히 동일하지 않으므로, 이 표는 엄밀한 leaderboard 비교가 아니라 문제 난이도와 성능 범위를 이해하기 위한 비교로 사용한다.
...
\begin{figure}[p]
    \centering
    \includegraphics[width=\linewidth,height=0.86\textheight,keepaspectratio]{figures/qualitative_grid_placeholder.png}
    \caption{256 best checkpoint 기반 qualitative examples. 각 예시는 input / D3Net output / GT 비교 패널이며, finger-hard, dust-medium, scratch-hard, water-hard, mixed-medium sample을 포함한다. The figure is height-limited to prevent page overflow and clipping.}
...
이에 비해 본 프로젝트의 main run은 epoch 34에서 best를 기록했고 epoch 64에서 early stopping되었다.
...
향후 연구에서는 official-scale long training 또는 pretrained D3Net, TTA/self-ensemble, difficulty-aware sampling, frequency-aware loss, perceptual/edge-aware loss, 그리고 AirNet/DiffUIR와 동일 split 및 동일 budget에서의 통제된 비교가 필요하다.
